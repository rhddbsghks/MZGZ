    ...
    <>
      <Flex m="auto">
        ...
        <Box flex="1" m="auto" overflow="auto" p="auto">
          ...
          <br />
          <Text fontSize="lg">
            <strong>소유자: </strong> {account}
          </Text>
          <Text fontSize="lg">
            <strong>시리얼번호: </strong> {serialNum}
          </Text>
          <br />
          {saleHistory && saleHistory.length > 0 ? (
            <>
              <Text fontSize="lg" marginBottom="2">
                <strong>
                  총 {saleHistory.length}건의 거래 내역이 있습니다.
                </strong>
              </Text>
              <Box>
                {saleHistory.map((v, _) => {
                  var time = new Date(
                    Date.parse(new Date(v.dealTime * 1000).toUTCString())
                  );
                  return (
                    <Text fontSize="lg">
                      <strong>거래 일시(가격): </strong>
                      {time.toLocaleString()} ({web3.utils.fromWei(v.dealPrice)}{" "}
                      ETH)
                    </Text>
                  );
                })}
